...
import { isChunkLoadError, reloadOnceForStaleChunk } from "@/lib/lazyWithReload";

type ErrorBoundaryState = {
  hasError: boolean;
  browserLevel: "modern" | "compatibility" | "unsupported";
  isChunk: boolean;
};

export default class ErrorBoundary extends React.Component<React.PropsWithChildren<object>, ErrorBoundaryState> {
  state: ErrorBoundaryState = { hasError: false, browserLevel: "modern", isChunk: false };

  static getDerivedStateFromError(error: unknown): ErrorBoundaryState {
    const level = typeof detectBrowserLevel === "function" ? detectBrowserLevel() : "modern";
    return { hasError: true, browserLevel: level, isChunk: isChunkLoadError(error) };
  }
          ...
          <div style={{ maxWidth: "28rem", width: "100%", textAlign: "center" }}>
            ...
            <p style={{ fontSize: "0.875rem", color: "#666", lineHeight: 1.6, marginBottom: "1.5rem" }}>
              {isUnsupported
                ...
                : isChunk
                  ? "Une nouvelle version de NACK a été déployée. Rechargez pour continuer."
